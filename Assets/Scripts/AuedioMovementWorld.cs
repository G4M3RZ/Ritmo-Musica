...
public class AuedioMovementWorld : MonoBehaviour {

    public AudioPeerV2 _audioPeerV2;
    ...
    private void Update()
    {
        if (_useScaleAnimation)
        {
            if (_useScaleCurve)
            {
                _scaleTimer += (_scaleAnimSpeed * _audioPeerV2._audioBand[_scaleBand]) * Time.deltaTime;
                if(_scaleTimer >= 1)
                {
                    _scaleTimer -= 1;
                }
            ...
            {
                _currentScale = Mathf.Lerp(_scaleAnimMinMax.x, _scaleAnimMinMax.y, _audioPeerV2._audioBand[_scaleBand]);
            }
        }
        ...
        if (_useLerping)
        {
            if (_isLerping)
            {
                _lerpPosSpeed = Mathf.Lerp(_lerpPosSpeedMinMax.x, _lerpPosSpeedMinMax.y, _lerpPosAnimCurve.Evaluate(_audioPeerV2._audioBand[_lerpPosBand]));
                ...
                if (_lerpPosTimer >= 1)
                {
                    ...
                    else //currentIteration has hit 0 or maxitaration
                    {
                        if (_repeat)
                        {
                            if (_invert)
